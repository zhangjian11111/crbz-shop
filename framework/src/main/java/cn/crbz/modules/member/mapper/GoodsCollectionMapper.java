...
public interface GoodsCollectionMapper extends BaseMapper<GoodsCollection> {
     ...
     * @return 商品收藏VO分页
     */
    @Select("select gc.id AS id,gs.id as sku_id,gs.goods_id as goods_id,gs.goods_name as goods_name,gs.thumbnail as image,gs.price,gs.market_enable AS market_enable from crbz_goods_collection gc INNER JOIN crbz_goods_sku gs ON gc.sku_id=gs.id    ${ew.customSqlSegment} ")
