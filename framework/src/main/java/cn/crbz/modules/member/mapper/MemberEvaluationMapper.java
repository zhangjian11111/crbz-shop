...
import cn.crbz.modules.member.entity.vo.StoreRatingVO;
...
public interface MemberEvaluationMapper extends BaseMapper<MemberEvaluation> {
     ...
     * @return 会员评价分页
     */
    @Select("select me.* from crbz_member_evaluation as me ${ew.customSqlSegment}")
     ...
     * @return 会员评价
     */
    @Select("select grade,count(1) as num from crbz_member_evaluation Where goods_id=#{goodsId} and status='OPEN' and delete_flag = false GROUP BY grade")
    ...
    @Select("SELECT round( AVG( delivery_score ), 2 ) AS delivery_score" +
            ...
            "FROM crbz_member_evaluation ${ew.customSqlSegment}")
     ...
     * @return 评价数量
     */
    @Select("SELECT goods_id,COUNT(goods_id) AS num FROM crbz_member_evaluation GROUP BY goods_id")
