...
public interface OrderStatisticsMapper extends BaseMapper<Order> {
     ...
     * @return 订单统计列表
     */
    @Select("SELECT DATE_FORMAT(create_time,'%Y-%m-%d') AS create_time,sum(flow_price) AS price FROM crbz_order " +
            " ${ew.customSqlSegment}")
     ...
     * @return 订单数量
     */
    @Select("SELECT count(0) FROM crbz_order ${ew.customSqlSegment}")
