...
import cn.crbz.modules.statistics.entity.vo.CategoryStatisticsDataVO;
import cn.crbz.modules.statistics.entity.vo.GoodsStatisticsDataVO;
import cn.crbz.modules.statistics.entity.vo.StoreStatisticsDataVO;
...
public interface StoreFlowStatisticsMapper extends BaseMapper<StoreFlow> {
     ...
     * @return 商品统计列表
     */
    @Select("SELECT goods_id,goods_name,SUM(final_price) AS price,SUM(num) AS num FROM crbz_store_flow ${ew.customSqlSegment}")
     ...
     * @return 分类统计列表
     */
    @Select("SELECT category_id,category_name,SUM(price) AS price,SUM(num) AS num FROM crbz_store_flow ${ew.customSqlSegment}")
     ...
     * @return 店铺统计列表
     */
    @Select("SELECT store_id AS storeId,store_name AS storeName,SUM(final_price) AS price,SUM(num) AS num FROM crbz_store_flow ${ew.customSqlSegment}")
     ...
     * @return 付款人数
     */
    @Select("SELECT count(0) AS num FROM (SELECT count(0) FROM crbz_store_flow " +
