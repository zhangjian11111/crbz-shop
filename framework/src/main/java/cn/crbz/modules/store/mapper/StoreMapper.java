...
public interface StoreMapper extends BaseMapper<Store> {
     ...
     * @return 店铺VO
     */
    @Select("select s.*,d.* from crbz_store s inner join crbz_store_detail d on s.id=d.store_id where s.id=#{id} ")
     ...
     * @return 店铺VO分页列表
     */
    @Select("select s.* from crbz_store as s ${ew.customSqlSegment}")
